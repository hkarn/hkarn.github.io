  ...
  render () {
    ...
    try {
      ...
      try {
        ...
        if (languages.includes(browserLang)) {
          defaultLang = translations[browserLang]
        }
      } catch (e) {
        defaultLang = translations['en']
      }
    }
    ...
    const theLoader = (style, icon, pulse, firstline, secondline) =>
      <div className="Loader" style={style}>
        <Textfit style={{height: '50%', width: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '2px'}} mode="single" forceSingleModeWidth={false} max={200} min={4} throttle={900}><FontAwesomeIcon icon={icon} pulse={pulse} /></Textfit>
        {firstline === defaultLang.firstlinedefault
          ? <Textfit style={{height: '45%', width: '100%', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '2px'}} mode="single" forceSingleModeWidth={false} max={40} min={4} throttle={900}><p className="loading">{firstline}</p></Textfit>
          : <Textfit style={{height: '20%', width: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '2px'}} mode="single" max={35} min={9} throttle={900}><p>{firstline}</p></Textfit>
        }
        {firstline === defaultLang.firstlinedefault ? null
          : <Textfit style={{height: '20%', width: '100%', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '2px'}} mode="single" max={35} min={8} throttle={900}>{secondline}</Textfit>
        }
